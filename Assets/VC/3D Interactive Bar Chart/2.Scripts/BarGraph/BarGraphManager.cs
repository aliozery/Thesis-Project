...
namespace BarGraph.VittorCloud
{
    public class BarGraphManager : GraphBox
    {
        ...
        public IEnumerator GenerateGraphBarWithAnimTypeOne(int xIndex, int zIndex, float yValue, float scaleFactor, float animSpeed, int ymin, int xMax, Color barColor)
        {
            ...
            if(bar.transform.parent.name[0]!='i'){
                bar.transform.parent.name="id"+counter;        //change name of each bar
                counter++;
            }
        ...
        public void GenerateBarWithAnimTypeTwo(int xIndex, int zIndex, float yValue, float scaleFactor, float animSpeed, int ymin, int xMax, Material barMaterial)
        {
            ...
            bar.transform.parent = ListOfGroups[zIndex].transform;
            if(bar.transform.parent.name[0]!='i'){
                bar.transform.name="id"+counter;        //change name of each bar group (each bar group is 1 row of data)
                counter++;
            }
        ...
        {
            GameObject example= GameObject.FindGameObjectWithTag("Bar_Graph");
            
            if (graphGenerator != null){
                graphGenerator.OnBarPointerDown.Invoke(Clickedbar);
                //insert overlay here, first find row corresponding to bar
            int temp_id=0;
            string temp_id_string="";
            for(int i=2;i<Clickedbar.transform.parent.name.Length;i++){
                temp_id_string+=Clickedbar.transform.parent.name[i];
            }
            temp_id=int.Parse(temp_id_string);
            string information= example.GetComponent<BarGraphExample>().OverlayData(temp_id-1);
            example.GetComponent<BarGraphExample>().DisplayOverlay(information);
            // GameObject.FindGameObjectWithTag("Info_Overlay_Bar").SetActive(true);
            // GameObject.FindGameObjectWithTag("Info_Overlay_Text_Bar").GetComponent<Text>().text=information;

            // Debug.Log(temp_id);

            // GameObject d_holder= GameObject.FindGameObjectWithTag("DataHolder");
            // string data_display= d_holder.GetComponent<dataholder>().data_list[temp_id-1].DisplayData();
            // GameObject.FindGameObjectWithTag("OverlayHandler").GetComponent<overlayhandler>().OpenPanel(data_display);
